...
import { TextProps } from "site/sections/CampaignTimerHero.tsx";
...
function CampaignTimer({
    ...
    const TimeComponent: preact.FunctionalComponent<TimeComponentProps> = (
        ...
        <div class="flex flex-col items-center">
            ...
            <span class="countdown font-normal text-xl lg:text-2xl">
                <span
                    class="text-5xl lg:text-[78px] leading-none font-light text-primary-content tracking-[-3px]"
                    id={`${id}::${time}`}
    ...
    return (
        <>
            <div>
                <div class="container justify-center mx-auto mt-11 lg:mt-[60px] flex flex-col lg:flex-row lg:items-center lg:gap-16 gap-4">
                    <div
                        id={`${id}::expired`}
                        class="hidden text-sm text-center lg:text-xl lg:text-left lg:max-w-lg"
                        dangerouslySetInnerHTML={{ __html: expiredText || "Expired!" }}
                        style={{ ...expiredTextProps }}
                    >
                    </div>
