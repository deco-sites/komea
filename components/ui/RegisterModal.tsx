...
function RegisterModal({ registerModal }: Props) {
    ...
    return (
        ...
        >
            <div className="bg-white rounded-[20px] p-8 lg:p-[80px] w-[90%] max-w-[582px] relative shadow-xl text-center">
                ...
                <h2 className="text-[#00363A] bg-[#EEFC58] px-3 py-1 rounded text-xl lg:text-[32px] font-bold inline-block mb-4 lg:mb-6 font-lektorat" dangerouslySetInnerHTML={{ __html: registerModal?.title ?? `Cadastro Confirmado` }} />
                <p className="text-[#5F6E82] mb-2 max-w-[376px] mx-auto" dangerouslySetInnerHTML={{ __html: registerModal?.subTitle ?? `Você está na sua lista de interessados para testar a Komea, mas lembre-se para usar o KOMEA, você precisa ter sua loja ativa na Loja Integrada — e o melhor momento para isso é agora.` }} />
                <p className="text-[#5F6E82]" dangerouslySetInnerHTML={{ __html: registerModal?.extraText ?? `<strong>Abra sua loja agora, é rápido, simples e fácil.</strong>` }} />
                <a
                    ...
                    target="_blank"
                >
                    {registerModal?.buttonText ?? 'Criar minha loja agora'}
                </a>
            </div>
        </div>
    )
