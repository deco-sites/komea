...
export default function HeroV3({ title, text, textProps, bulletPoints, cta = [], media, hubspotForm, distanceBetweenTitleAndText, container, ctaPlacement, sectionBackground, lcp, floatingImage }: Props) {
  ...
  return <div class="px-5 lg:px-0 relative py-12" style={{ paddintTop: container?.marginTop, paddingBottom: container?.marginBottom }}>
    <div
      class={`max-w-[1288px] relative z-10 mx-auto rounded-[20px] overflow-hidden p-[30px] lg:p-12 flex gap-5 lg:gap-y-10 lg:flex-nowrap items-center justify-center ${mediaPlacement[media?.placement || "right"]}`}
      ...
      <AnimateOnShow animation="animate-fade-up50" divClass={`${(media?.use && media.placement != "bellow" && media.placement != "above") && 'max-w-[448px]'} w-full flex flex-col gap-10`} style={{ animationDuration: '1s', maxWidth: title?.titleMaxWidth }}>
        ...
        {hubspotForm?.show && <div class="pt-5 md:pt-10 max-w-[792px] w-full mx-auto">
          ...
          <div id={modalId} class="fixed top-0 left-0 h-screen w-screen flex items-center justify-center bg-black bg-opacity-50 z-[60] overflow-auto hidden px-6">            
                <div class="bg-white lg:rounded-[30px] p-7 lg:p-[60px] animate-pop-up relative max-w-[840px] w-full" style={{ animationDuration: "0.3s" }}>
                    <button class="text-primary font-black p-2.5 absolute top-6 right-7" hx-on:click={useScript(closeModal, modalId)}>
                        <svg width="42" height="42" viewBox="0 0 42 42" class="fill-current" style={{color: hubspotForm.modal?.titleTextProps?.color || "black"}} xmlns="http://www.w3.org/2000/svg">
    ...
    {sectionBackground?.use == "image" && sectionBackground.image?.src && <Image
      ...
      class={`absolute -z-50 top-0 left-0 h-full w-full object-cover ${backgroundMediaPlacement[sectionBackground.postition || "center"]}`}
    ...
    {sectionBackground?.use == "video" && sectionBackground.video && <video width={1280} height={720} autoPlay playsInline muted loading={lcp ? "eager" : "lazy"} loop
      class={`object-cover absolute -z-50 top-0 left-0 h-full w-full ${backgroundMediaPlacement[sectionBackground.postition || "center"]}`}>
